...
class NuevoProyectoSupabaseUser extends BaseAuthUser {
  NuevoProyectoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> nuevoProyectoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = NuevoProyectoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
